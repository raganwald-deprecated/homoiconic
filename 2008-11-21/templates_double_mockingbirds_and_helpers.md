...
Today we are going to meet another combinator that does not conserve its arguments, the Double Mockingbird. But where a kestrel erases one of its arguments, the double mockingbird *duplicates* both of its arguments. In logic notation, `M`<sub>2</sub>`xy = xy(xy)`. Or in Ruby:
